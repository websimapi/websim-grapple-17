...
export class Car {
    ...
    getFrameData() {
        ...
        // [px, py, pz, qx, qy, qz, qw, hx, hy, hz, stateIndex]
        const p = this.position;
        const q = this.mesh.quaternion;
        const h = this.hookPosition;
        
        // Map state to int for compactness
        let s = 0;
        if (this.grappleState === 'FIRING') s = 1;
        else if (this.grappleState === 'ATTACHED') s = 2;
        else if (this.grappleState === 'RETRACTING') s = 3;

        // Rounding to 3 decimals to save space if this goes to JSON string
        return [
            Number(p.x.toFixed(3)), Number(p.y.toFixed(3)), Number(p.z.toFixed(3)),
            Number(q.x.toFixed(4)), Number(q.y.toFixed(4)), Number(q.z.toFixed(4)), Number(q.w.toFixed(4)),
            Number(h.x.toFixed(3)), Number(h.y.toFixed(3)), Number(h.z.toFixed(3)),
            s
        ];
    }
    ...
    applyFrameData(data) {
        ...
        this.mesh.quaternion.set(data[3], data[4], data[5], data[6]);
    ...
    updateGrappleVisuals() {
        ...
        // Hook
        this.hookMesh.position.copy(endPos);
        this.hookMesh.lookAt(startPos); // Look back at car
    }
    ...
    update(dt, input, trackManager) {
        ...
        if (input.mouseDown) {
            ...
            if (this.grappleState === 'IDLE' && grappleInfo.target && grappleInfo.distance < 60) {
                this.fireGrapple(grappleInfo.target);
            }
        } else {
        ...
        // State Machine Logic
        this.updateGrapplePhysics(effectiveDt);
        ...
        if (this.grappleState === 'ATTACHED' && this.grappleTarget && !this.grappleTarget.type) {
            ...
            // Linear Motion
            const targetSpeed = this.boostTimer > 0 ? 75 : 35;
            this.speed = THREE.MathUtils.lerp(this.speed, targetSpeed, effectiveDt * 2);
            
            // Calculate horizontal movement
            const moveStep = this.direction.clone().multiplyScalar(this.speed * effectiveDt);
            ...
            if (trackState.onTrack) {
                ...
                this.position.y += diff * 25 * effectiveDt;
                ...
                // Falling
                this.verticalVelocity -= 40 * effectiveDt; // Gravity
                this.position.y += this.verticalVelocity * effectiveDt;
        ...
        this.mesh.quaternion.slerp(targetQuat, effectiveDt * 20);

        // Rotating item visuals if we are tracking them
        if (trackManager.items) {
            trackManager.items.forEach(item => {
                if (item.active) {
                    item.mesh.rotation.y += dt * 2;
                    item.mesh.rotation.x += dt * 1;
                }
            });
        }
    }
    ...
    updateGrapplePhysics(dt) {
        ...
        if (this.grappleState === 'FIRING') {
            ...
            if (dist <= travelDist) {
                ...
                // IF TARGET IS ITEM: Trigger collection immediately
                if (this.grappleTarget && this.grappleTarget.type) {
                    this.collectItem(this.grappleTarget);
                }
            } else {
                // Move towards target
