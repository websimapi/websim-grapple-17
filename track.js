...
export class TrackManager {
    constructor(scene) {
        ...
        this.segmentHistory = []; // Store generation parameters for replay
    ...
    addSegment(params) {
        ...
        this.segmentHistory.push(params);
        ...
        // Random Items on Straights
        if (type === 'straight' && Math.random() < 0.2) {
            this.addItem(midpoint, slope);
        }
    ...
    rebuildFromHistory(history) {
        ...
        this.segmentHistory = []; // addSegment will repopulate this matches playback

        // Rebuild
        for(const params of history) {
            this.addSegment(params);
        }
    }
    ...
    addItem(midpoint, slope) {
        ...
        pos.y += 6; // Float above road
        
        // Randomly offset to side
        const perp = new THREE.Vector3(-this.currentDir.z, 0, this.currentDir.x);
        pos.add(perp.multiplyScalar((Math.random() - 0.5) * 10));
    ...
    getNearestGrappleTarget(position) {
        let nearest = null;
        let minDist = Infinity;
        let isItem = false;

        // Check Posts
        const postCheckCount = Math.min(this.posts.length, 20);
        const postStartIndex = this.posts.length - postCheckCount;
        for (let i = postStartIndex; i < this.posts.length; i++) {
            const post = this.posts[i];
            if (Math.abs(post.position.y - position.y) > 20) continue;
        ...
        // Check Items
        const itemCheckCount = Math.min(this.items.length, 10);
        const itemStartIndex = Math.max(0, this.items.length - itemCheckCount);
        for (let i = itemStartIndex; i < this.items.length; i++) {
        ...
        return { target: nearest, distance: minDist, isItem: isItem };
    }
